...
class CheckpointManager(object):
    ...
    # [修改] 增加 bd_score 参数，默认为 None 以兼容普通训练
    def save_bd(self, model, args, score, bd_score=None, others=None, step=None):

        # 格式化文件名：ckpt_clean{分数}_bd{分数}_{步数}.pt
        if step is None:
            if bd_score is not None:
                fname = 'ckpt_clean%.6f_bd%.6f.pt' % (float(score), float(bd_score))
            else:
                fname = 'ckpt_%.6f_.pt' % float(score)
        else:
            if bd_score is not None:
                # C 代表 Clean CD, B 代表 BD Control CD
                fname = 'ckpt_C%.6f_B%.6f_%d.pt' % (float(score), float(bd_score), int(step))
            else:
                fname = 'ckpt_%.6f_%d.pt' % (float(score), int(step))
